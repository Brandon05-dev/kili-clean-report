  ...
  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8 p-2 sm:p-4 lg:p-6">
      ...
      <div className="text-center py-4 sm:py-6 lg:py-12 bg-gradient-to-r from-emerald-50 via-green-50 to-teal-50 rounded-lg sm:rounded-xl lg:rounded-2xl border border-green-200">
        ...
        <p className="text-sm sm:text-base lg:text-lg text-green-700 max-w-3xl mx-auto font-medium px-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-2 sm:mb-4">
                      <h3 className="text-xs sm:text-sm font-semibold text-gray-700 uppercase tracking-wide truncate pr-2">{stat.title}</h3>
                      <div className={`${colors.iconBg} p-2 sm:p-3 rounded-lg sm:rounded-xl shadow-md flex-shrink-0`}>
                    ...
                    <div className="flex items-baseline space-x-2 sm:space-x-3 mb-2 sm:mb-3">
                      <span className={`text-2xl sm:text-3xl lg:text-4xl font-bold ${colors.text}`}>
                        {stat.value}
                      </span>
                      {stat.value > 0 && (
                        <Badge className={`bg-gradient-to-r ${colors.accent} text-white border-0 shadow-sm text-xs`}>
                          Active
                        </Badge>
                      )}
                    </div>
                    
                    <p className="text-xs sm:text-sm text-gray-600 mb-2 sm:mb-4 font-medium">{stat.description}</p>
                    
                    <Button
                      variant="ghost"
                      size="sm"
                      className={`p-0 h-auto ${colors.text} hover:${colors.bg} font-semibold transition-all duration-200 text-xs sm:text-sm`}
                      onClick={(e) => {
                        e.stopPropagation();
                        stat.action();
                      }}
      ...
      <Card className="border-0 shadow-lg bg-white/90 backdrop-blur-sm">
        ...
        <CardContent className="p-4 sm:p-6 lg:p-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
            <Button
              variant="outline"
              className="h-16 sm:h-20 lg:h-24 flex flex-col items-center justify-center space-y-2 sm:space-y-3 border-emerald-200 hover:bg-emerald-50 hover:border-emerald-300 transition-all duration-200 group text-xs sm:text-sm"
              onClick={() => onNavigate('active')}
            >
              <div className="p-1 sm:p-2 bg-emerald-100 rounded-lg group-hover:bg-emerald-200 transition-colors">
                <FileText className="w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7 text-emerald-600" />
              </div>
              <span className="font-semibold text-emerald-700 text-center leading-tight">Manage Active Reports</span>
            </Button>
            
            <Button
              variant="outline"
              className="h-16 sm:h-20 lg:h-24 flex flex-col items-center justify-center space-y-2 sm:space-y-3 border-green-200 hover:bg-green-50 hover:border-green-300 transition-all duration-200 group text-xs sm:text-sm"
              onClick={() => onNavigate('teams')}
            >
              <div className="p-1 sm:p-2 bg-green-100 rounded-lg group-hover:bg-green-200 transition-colors">
                <Users className="w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7 text-green-600" />
              </div>
              <span className="font-semibold text-green-700 text-center leading-tight">Manage Teams</span>
            </Button>
            
            <Button
              variant="outline"
              className="h-16 sm:h-20 lg:h-24 flex flex-col items-center justify-center space-y-2 sm:space-y-3 border-teal-200 hover:bg-teal-50 hover:border-teal-300 transition-all duration-200 group text-xs sm:text-sm"
              onClick={() => onNavigate('completed')}
            >
              <div className="p-1 sm:p-2 bg-teal-100 rounded-lg group-hover:bg-teal-200 transition-colors">
                <CheckCircle className="w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7 text-teal-600" />
              </div>
              <span className="font-semibold text-teal-700 text-center leading-tight">View Completed</span>
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-0 shadow-lg bg-white/90 backdrop-blur-sm">
        ...
        <CardContent className="p-4 sm:p-6 lg:p-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
            <div>
              <h4 className="font-bold text-gray-900 mb-4 sm:mb-6 text-base sm:text-lg">Workflow Status</h4>
              <div className="space-y-3 sm:space-y-4">
                <div className="flex justify-between items-center p-3 sm:p-4 rounded-lg bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200">
                  <span className="text-xs sm:text-sm font-medium text-yellow-800">Reports Pending Assignment</span>
                  <Badge className={`text-xs ${stats.newReports > 0 ? 
                ...
                <div className="flex justify-between items-center p-3 sm:p-4 rounded-lg bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200">
                  <span className="text-xs sm:text-sm font-medium text-emerald-800">Team Capacity</span>
                  <Badge className="bg-gradient-to-r from-emerald-500 to-green-500 text-white shadow-sm text-xs">
                    {stats.availableTeams} Teams Active
                  </Badge>
                </div>
                <div className="flex justify-between items-center p-3 sm:p-4 rounded-lg bg-gradient-to-r from-teal-50 to-cyan-50 border border-teal-200">
                  <span className="text-xs sm:text-sm font-medium text-teal-800">Today's Progress</span>
                  <Badge className="bg-gradient-to-r from-teal-500 to-cyan-500 text-white shadow-sm text-xs">
            ...
            <div>
              <h4 className="font-bold text-gray-900 mb-4 sm:mb-6 text-base sm:text-lg">Next Steps</h4>
              <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-gray-700">
                ...
                <div className="p-2 sm:p-3 rounded-lg bg-emerald-50 border border-emerald-200">
                ...
                <div className="p-2 sm:p-3 rounded-lg bg-teal-50 border border-teal-200">
